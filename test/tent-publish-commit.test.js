...
// This test simulates the publishTrail() success-branch handling focused on
// commit short-circuit vs waitForPublishedRoute fallback. It does not execu
// te browser code; it mirrors the minimal response-handling logic used in
// the client to validate behavior across CASE 1..9.
...
test('publish commit short-circuit cases 1..8', async () => {
  ...
  // CASE 9: IMS vs BACBC behavior parity
...
test('tent-app.js contains the commit short-circuit snippet', () => {
  const fs = require('node:fs');
  const path = require('node:path');
  const source = fs.readFileSync(path.join(__dirname, '..', 'tent-app.js'), 'utf8');
  assert.match(source, /If the server returned a non-empty commit SHA, treat publish as successful/);
  assert.match(source, /const commit = result && typeof result.commit === "string" \? result.commit.trim\(\) : null/);
});
