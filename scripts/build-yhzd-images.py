...
Drop original JPEG/PNG files into assets-src/yhzd/.
...
try:
    ...
    raise SystemExit("Pillow is required. Install once with: python3 -m pip install Pillow")
ROOT=Path(__file__).resolve().parents[1]
SOURCE_DIR=ROOT/"assets-src"/"yhzd"
OUTPUT_DIR=ROOT/"images"/"yhzd"
WIDTHS=(480,960)
TARGET_BYTES={480:70000,960:180000}
QUALITIES=(78,74,70,66,62,58,54,50)
EXTENSIONS={".jpg",".jpeg",".png",".webp",".tif",".tiff"}
def image_id(path:Path)->str:return hashlib.sha256(path.read_bytes()).hexdigest()[:12]
def resize_only(image:Image.Image,target_width:int)->Image.Image:
    width,height=image.size
    if width<=target_width:return image.copy()
...
def main()->int:
    ...
    manifest={"version":1,"images":[]};seen_ids=set()
    for source in sources:
        ...
        width,height=image.size
        record={"id":uid,"orientation":"portrait" if height>width else "landscape","width":width,"height":height}
        for target_width in WIDTHS:
            ...
            record[str(target_width)]={"src":f"/images/yhzd/{output_name}","width":resized.width,"height":resized.height}
